    ...
    t ||
      (Log_1.Log.CheckWarn() &&
        Log_1.Log.Warn(
          ...
          ["path", e],
          ["reason", "未在 /Game/Aki/WwiseAudio/Events/ 路径下或命名不符合规范"]
        )),
    t?.groups?.name
  );
}
...
class AudioSystem {
  ...
  static PostEvent(i, o, n) {
    this.KunLog(`AudioPostEvent ${i}`);
    this.KunLog(this.StackTrace());
    return !i || i.length < 1
  ...
  static SeekOnEvent(e, t, i = {}) {
    ...
    !e || e.length < 1
      ? Log_1.Log.CheckError() &&
        Log_1.Log.Error("Audio", 43, "[Core.AudioSystem] 空的音频事件event参数")
      : void 0 === i.Handle
